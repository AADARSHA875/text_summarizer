...
export default function Register() {
  ...
  const submit = async (e) => {
    ...
    setBusy(true);
    try { await register(form.email, form.username, form.fullName, form.password); }
        ...
        <div className="card" style={{ padding: 32 }}>
          ...
          <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <button className="btn btn-amber" type="submit" disabled={busy} style={{ marginTop: 8, width: "100%", justifyContent: "center", padding: "11px" }}>
              {busy ? <span className="spin" style={{ display: "inline-block", width: 14, height: 14, border: "2px solid #0e0e0e", borderTopColor: "transparent", borderRadius: "50%" }} /> : "Create account →"}
            </button>
          </form>
        </div>
